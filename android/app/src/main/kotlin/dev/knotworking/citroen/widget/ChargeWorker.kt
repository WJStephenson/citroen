 ...
 *
 * Never retries. The next run is fifteen minutes away and reads a cache that
 * updates on its own schedule anyway, so a backoff would only spend battery
 * racing a number that has not changed. A failure is not dropped either: it is
