...
class InitialSetup:
    ...
    def connect(self, code, config_prefix=""):
        ...
        # Charge control
        # Patched: preserve any existing per-VIN charge_config.json settings
        # (percentage_threshold, stop_hour) across reconnects/re-pairing --
        # upstream used to unconditionally overwrite them with defaults here,
        # which silently wiped a configured stop_hour every time the OTP/
        # pairing flow re-ran (e.g. after a long connectivity outage).
        charge_config_name = config_prefix + "charge_config.json"
        try:
            charge_controls = ChargeControls.load_config(self.psacc, name=charge_config_name)
        except (FileNotFoundError, json.JSONDecodeError):
            charge_controls = ChargeControls(charge_config_name)
        for vehicle in res:
            if charge_controls.get(vehicle.vin) is None:
                charge_controls[vehicle.vin] = ChargeControl(self.psacc, vehicle.vin, 100, [0, 0])
