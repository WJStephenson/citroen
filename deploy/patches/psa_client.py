    ...
    def record_info(self, car: Car):  # pylint: disable=too-many-locals
        ...
        if car.has_battery():
            ...
            try:
                ...
                logger.debug(ex)
            try:
                # Prefer capacity retention (true battery degradation/SOH) over internal resistance
                health = getattr(getattr(electric_energy_status, "battery", None), "health", None)
                soh = getattr(health, "capacity", None) if health else None
                if soh is None and health:
                    soh = getattr(health, "resistance", None)
                if soh is not None:
